...
def api_start(server_dir, cfg, log=print):
    """非交互启动 (GUI 用): 按 cfg 下载并启动, 日志走 log 回调
    成功返回 True, 失败返回 False"""
    def out(msg):
        log(re.sub(r"\x1b\[[0-9;]*m", "", msg))

    old = read_pid(server_dir)
    if old and is_running(old["pid"]):
        out(f"[错误] 服务器已在运行 (PID {old['pid']})")
        return False
    ...
    if stype == "fabric":
        ...
        if not os.path.exists(os.path.join(server_dir, "run.sh")) and \
           not os.path.exists(os.path.join(server_dir, "run.bat")):
            out(f"正在安装 {stype} (下载依赖库, 可能需要几分钟)...")
            reuse_libraries(server_dir)
            if not get_mod_installer(cfg["version"], stype, server_dir,
                                     java_bin, cfg.get("mod_version")):
                out(f"[错误] {stype} 安装失败")
                return False
    ...
    out(f"正在启动: MC {cfg['version']} 端口 {cfg['port']} "
        f"内存 {cfg.get('ram', '2G')}")
    p = start_server(server_dir, cfg)
    out(f"已启动! PID {p.pid}")
    ...
    if cfg.get("gamerules"):
        ...
        if wait_ready(server_dir):
            for rule, val in cfg["gamerules"].items():
                send_console(server_dir, f"gamerule {rule} {val}")
            out("已设置游戏规则: " + ", ".join(
                f"{k}={v}" for k, v in cfg["gamerules"].items()))
        else:
            out("等待就绪超时, 游戏规则未注入(可用 console 命令手动设置)")
    return True
...
def do_config(server_dir):
    """修改服务器设置: 内存/端口/人数/全部项, 改完保存并提示重启生效"""
    cfg = load_cfg(server_dir)
    if not cfg:
        sys.exit(c(f"[错误] 没有找到配置: {server_dir}", "red"))
    props = dict(cfg["props"])
    while True:
        port_now = str(cfg.get("port") or props.get("server-port", "25565"))
        motd = props.get("motd", "")
        motd_lines = motd.split("\n") if motd else []
        name_now = motd_lines[0] if motd_lines else ""
        sub_now = motd_lines[1] if len(motd_lines) > 1 else ""
        print(c(f"\n[修改服务器设置] {server_dir}", "cyan"))
        print(c(f"  当前: 端口 {port_now} · 内存 {cfg.get('ram', '?')} · "
                f"人数 {props.get('max-players', '20')}", "dim"))
        print(f"  1. 内存上限      = {c(cfg.get('ram', '?'), 'green')}")
        print(f"  2. 端口          = {c(port_now, 'green')}")
        print(f"  3. 最大人数      = {c(props.get('max-players', '20'), 'green')}")
        print(f"  4. 服务器名字    = {c(name_now or '(未设置)', 'green')}"
              f"{('  副标题 = ' + c(sub_now, 'green')) if sub_now else ''}")
        print(f"  5. 高级选项(编辑全部项)")
        print(c("  输入 1-5 修改, 0 退出", "yellow"))
        ans = ask("  修改")
        changed = False
        if ans in ("", "0"):
            break
        if ans == "1":
            # 内存: 空闲70%以下档位, 可自定义, 0 取消 (不改动)
            total = total_mem_mb()
            avail = avail_mem_mb()
            cap = int((avail or 2048) * 0.7)
            tiers = [1024, 2048, 4096, 8192, 16384, 32768, 65536]
            ...
            for i, t in enumerate(opts):
                mark = c("(推荐)", "green") if i == len(opts) - 1 else ""
                print(f"    {i+1}. {t // 1024}G {mark}")
            print("    也可以直接输入, 如 4G/512M (纯数字按G算, 0 取消)")
            while True:
                a = ask("  选择", str(len(opts)))
                if a == "0":
                    break
                if a.isdigit() and 1 <= int(a) <= len(opts):
                    cfg["ram"] = f"{opts[int(a) - 1] // 1024}G"
                    changed = True
                    break
                if re.match(r"^\d+[GM]?$", a, re.I):
                    cfg["ram"] = a.upper() + ("G" if not re.search(r"[GM]", a, re.I) else "")
                    changed = True
                    break
                print(c("  无效, 请输入序号或如 4G/512M (0 取消)", "red"))
        elif ans == "2":
            while True:
                port = ask("  新端口", port_now)
                if port == "0":
                    break
                if not port.isdigit() or not 1 <= int(port) <= 65535:
                    print(c("  端口无效, 请输入 1-65535 (0 取消)", "red"))
                    continue
                # 端口占用检测: 被占用自动换空闲端口
                s = socket.socket()
                try:
            ...
            while True:
                n = ask("  最大人数", str(props.get("max-players", "20")))
                if n == "0":
                    break
                if n.isdigit() and 1 <= int(n) <= 100000:
                    props["max-players"] = n
                    changed = True
                    break
                print(c("  无效, 请输入正整数 (0 取消)", "red"))
        elif ans == "4":
            # 服务器名字和副标题 (游戏内服务器列表看到的两行)
            print(c("  服务器名字 = 游戏界面看到的主标题", "dim"))
            print(c("  副标题 = 名字下面的一行小字 (留空 = 不要副标题, 0 = 取消)", "dim"))
            name = ask("  服务器名字", name_now or "1b1t Server")
            if name == "0":
                continue
            sub = ask("  副标题", sub_now)
            if sub == "0":
...
def usage():
  ...
  config <目录>        修改服务器设置(内存/端口/人数/全部项)
